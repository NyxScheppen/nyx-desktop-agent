...
失败后，后续有效提问仍被抑制。
**教训**：异步任务的“已选中”不等于“用户可见产出成功”；只延后写时间戳又会让并发段落
重复占用同一预算。
**怎么做**：用独立的 pending 标记防止并发重复，只有合法问题成功提交后才写入冷却；异常、
取消和未创建任务都必须释放 pending 且不消耗预算。
**影响的文件/决策**：`nyx/reading/facade.py`、`nyx/reading/companions.py`、
`nyx/reading/impulse.py` 及阅读回归测试。

### 2026-09-22: 主动提问的频率预算必须跨提问类型共享

**来源**：读书冲动引擎对四类问题分别判阈值、分别记冷却；同一段富文本可同时启动四次
问题 LLM。原有跨段冷却测试还因第二段不存在而误通过。
**教训**：并发槽只限制执行并发，不限制用户可见产出；“每种类型各自冷却”会被类型轮换绕过，
而只测缺失段落会把提前返回误认为冷却生效。
**怎么做**：把同一用户可感知行为的频率预算放在共享冷却上，每段从候选中只选一个最高分
问题；回归测试必须使用真实存在的连续段落，并覆盖冷却边界和联想共存。
**影响的文件/决策**：`nyx/reading/impulse.py`、`nyx/reading/facade.py`、
`docs/specs/12-reading-system.md`、`docs/facts/reading-system-facts.md`、阅读测试。

### 2026-09-21: 事实来源与批量映射必须同时收紧

**来源**：事实层审查发现前台窗口标题可伪造用户偏好、未白名单谓词会丢失 functional 语义、
别名解析全表扫描，以及知识点逐条触发事实抽取 LLM。
**教训**：观察型画像不是用户断言，不能进入事实抽取；解析器必须尊重模型明确的关系模式；
别名要用规范化索引表而不是在共享数据库锁内扫描 JSON；批量来源必须携带稳定索引并在落库前
映射回具体 Memory，批量失败仍回退到逐条确定性候选。
**怎么做**：`user_observation` 直接跳过事实抽取，偏好 fallback 也要求主语锚点；新增
`memory_entity_alias` 迁移与索引；`remember_knowledge` 多项合并一次 `fact_extraction`，按
`memory_index` 分发并保持 best-effort 降级。
**影响的文件/决策**：`nyx/memory/facts.py`、`nyx/memory/facade.py`、`nyx/db.py`、06-memory-system。

### 2026-09-21: 通用事实抽取必须携带来源范围，并与记忆事务解耦

**来源**：事实层从“用户属性”扩展为通用实体关系图时，知识/读书文本中的书中人物、
引用和 Nyx 自述容易被错误归因给用户；durable 活动记忆又不能在 SQLite 事务持锁期间等待 LLM。
**教训**：事实抽取 prompt 必须同时提供 memory kind、来源名称和说话者归因规则，解析器对
不明确主体采取跳过策略；LLM 抽取放在事务外，事务内只应用已解析候选。事实更新需要 savepoint，
即使事实 SQL 失败也不能回滚已经合法写入的原始记忆。
**影响的文件/决策**：`nyx/memory/facts.py`、`nyx/memory/facade.py`、`nyx/db.py`
...
### 2026-09-21: 事实抽取必须限制来源、同刻折叠并在 SQL 侧有界召回

**来源**：时间事实层代码审查发现同一记忆可产生互斥当前事实、书籍/知识文本可被归因到用户，
以及每条消息先全表物化有效事实。
**教训**：事实层的错误归因比漏抽取更危险；同一时间点的候选不能直接并存；召回先全量读再在
Python 过滤会随事实规模线性放大每条消息成本。
**怎么做**：只从允许的用户记忆类型和显式用户主语抽取；同一 subject+predicate+时间点按最后出现
状态折叠；查询先 SQL 过滤直接命中实体/谓词/对象，再按实体展开并设置上限；普通消息在表达层先做
查询门控。
