...
| ~~发呆 `{summary}` 回读 reflect 产出~~ ✅ 已落地 | — | B2：`IDLE_REFLECTION` result 带回反思 summary；阅读重读则发布 durable `REFLECTION`，统一由 `inner_life.reflection` consumer 执行 |
...
> 这些是**设计边界**（不是「未来要做」），故不进本清单、保留在 CLAUDE.md「安全」/ 各 spec：
