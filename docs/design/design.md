...
| 共同浏览 | 用户怎样在 Nyx 窗口内和她一起浏览网页，并把允许读取的页面经历留下来？ |
...
```mermaid
flowchart LR
    World[用户消息、电脑状态、时间、阅读内容、网页] --> Bus[事件与运行时]
    Bus --> Inner[内在生命]
    Bus --> Desire[欲望]
    Desire --> Activity[活动]
    Inner --> Expression[表达]
...
长期欲望还会记录它关联的价值观、进度和强度。反思可以把候选欲望与四个受控价值观键关联，
满足相关短期欲望时推进长期进度并让强度缓慢衰减；这些字段已经持久化并参与回写，但当前
尚未接入长期排序、短期 prompt 或活动决策，属于 V3 路线图中的后续深化点。
...
共同浏览把“和 Nyx 一起上网”做成窗口内的独立体验，而不是把网页伪装成书，也不是把
Nyx 的自由探索活动搬到用户浏览器里。中间区域是一个受控的单标签页浏览器，左侧聊天始终
保留；用户可以在看到页面的同时让 Nyx 看当前正文、讨论选区，或等待她对页面产生碎碎念、
提问和联想。

远程网页运行在隔离的原生 child WebView 中。地址栏、导航、授权提示和聊天属于受信任 UI，
网页只占据自己的内容矩形；网页内容被当作不可信材料，不能改变 Nyx 的人格、工具规则或
知识边界。Rust 负责导航安全检查、隐私门和页面快照，Python 负责 durable checkpoint、
陪伴表达、逐页整合和记忆写入，前端只消费展示 DTO 和事件。

登录态通过浏览专用的持久 profile 保存。同标签登录可以继续浏览，OAuth/SSO 弹窗只在用户
明确允许时临时打开，并与主浏览 child 共享隔离 profile；认证页、回调页和密码控件页面只
用于暂停采集，不保存正文、表单值或 URL 敏感参数。外部提供方拒绝嵌入或平台无法保证弹窗
隔离时，系统明确失败，不把外部浏览器的登录结果假装同步回来。

页面记忆遵循“先 checkpoint、后表达、再整合”的顺序。允许采集的页面正文、选区和 Nyx 的
浏览输出先留在本地 durable 状态；离页、撤销授权、进入认证模式或关闭会话时，旧页面被冻结，
未完成的输出等任务结束后再封口。整合成功后，浏览经历写入专用的长期记忆类型；刷新、后退
或同会话回到同一页面不会重复记忆，不同会话再次访问则可以留下新的共同经历。

共同浏览与普通聊天、阅读、自由探索的边界是显式的：聊天只有在当前 page id 仍有效时才会
带网页上下文；页面失效会得到明确 fallback，而不会静默退化成没有网页材料的普通回答。网络
不可用、正文不稳定、登录态不明或授权被暂停时，页面仍可显示，但不会伪造正文、总结或记忆。
...
```mermaid
flowchart LR
    Open[打开共同浏览] --> Child[创建隔离 child WebView]
    Child --> Navigate[导航并检查 URL/Origin]
    Navigate --> Privacy{公开内容页?}
    Privacy -- 否 --> Pause[暂停采集并提示登录/敏感状态]
    Privacy -- 是 --> Checkpoint[保存页面 checkpoint]
    Checkpoint --> Companion[碎碎念、提问或联想]
    Companion --> Chat[进入左侧聊天和事件历史]
    Checkpoint --> Integrate[离页后整合页面经历]
    Integrate --> Memory[写入浏览长期记忆]
    Pause --> Login[用户明确登录或授权]
    Login --> Navigate
```

这条路径的关键不是“网页能打开”，而是把网页上下文、用户授权和记忆生命周期接起来：
浏览器负责看到什么，Rust 负责哪些内容可以离开浏览器，后端负责哪些经历最终成为 Nyx 的
记忆。任何一环失败都保留可见状态和重试入口，不把失败包装成成功的共读体验。
