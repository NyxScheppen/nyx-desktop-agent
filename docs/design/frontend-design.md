...
- **对话/设置切换**（全屏模式内）：默认「立绘 + 对话框」，对话框头部「设置」按钮切到「立绘 + 设置标签页」（背景/记忆/Eval），设置面板「返回对话」退回。
- **内心世界抽屉**：观测面板（内在/欲望/活动/产出/叙事/资料）收在对话框右侧的「内心世界」抽屉里，默认收起，点对话框头部「内心」按钮滑出。
- 一键切换。
...
> 观测面板（内在/欲望/活动/产出/叙事/资料）收在「内心世界」右侧抽屉里（§2），默认收起，点对话框头部「内心」滑出；记忆/eval 留在「设置」。枚举值一律转中文上屏（`exploration → 发现` 等），未知键回退原值。
