...
- 前端游戏陪玩首版通过 `gameCompanionStore` 接收 `GAME_*` SSE、hydrate durable checkpoint，并在“陪玩”面板显示当前阶段、对白、选项和 pause/resume/stop/choice 操作；独立 companion 窗口只挂载陪玩面板，不启动全局 SSE、presence 或主窗口恢复轮询。hydrate 只接受当前 session 的不旧 revision，不会用断线恢复的旧 checkpoint 覆盖已收到的 SSE observation；`stale_choice` 会先重新 hydrate，再保留明确冲突提示。SSE 重连后由主窗口 `App` 重拉当前 game activity/session；恢复只选择 running/paused 活动，迟到的同 session `game_session_started` 不回退前端状态，暂停/恢复/结束和选择确认都在 UI 与后端校验当前 revision/status。主窗口可通过 `game_companion_open` 打开独立透明置顶陪玩窗；Rust 已接入 `game_list_windows`，在后台线程枚举 Windows 上可见、非最小化且有客户区的非 Nyx 顶层窗口，并返回 HWND/PID/进程启动时间、物理客户区、DPI 和前台标记。companion 窗口 Destroyed 时向主窗口发送 `game_companion_lost`，主窗口按当前 revision 将 session 暂停。WGC 捕获和 frame bridge 的原生产生仍待后续 Tauri 接线。
