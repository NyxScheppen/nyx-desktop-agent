...
turnPage(dir: 1 | -1): Promise<void>            // userPosition += dir（clamp [1, total]）→ 存进度 + 前翻触发 evaluateImpulse + 必要时翻窗口
...
- **进度持久化后写**：翻页 `turnPage` 每次 `putProgress(userPosition, nyxPosition, readingSpeed)`（fire-and-forget，失败静默、下次翻页重写覆盖）；`nyxPosition` 由追赶循环推进，也随下次 `putProgress` 落库（后端已定「state 是 value 派生」不存派生态，前端把最新 nyx 位置随进度写回即可）。
...
- **`totalParagraphs` 来自书架列表项**：后端 `GET /api/progress` 不回 total、段落窗口只回窗口内段，故唯一现成来源是 `BookListItem.total_paragraphs`；`openBook` 用 `books.find(b => b.id === bookId)` 落 `readerStore.totalParagraphs`。**前置 `books` 已加载**（书架点书天然已 `loadBooks`；深链/刷新先 `loadBooks` 再 `openBook`），否则 `totalParagraphs=0`、clamp 失效——`turnPage` 需 `totalParagraphs>0` 守卫，0 时不推进。
...
- **触发/停止**：`openBook`（恢复后 nyx 落后）与 `turnPage`（前翻拉开差距）调 `startCatchup`；`closeBook`/`stopCatchup` `clearTimeout`；段落窗口里找不到 `nyxPosition` 段（未加载）时按 `MIN_CATCHUP_SEC=1` 保底节奏，不阻塞。
- **重入安全**：`startCatchup` 先 `clearTimeout` 旧 timer 再排新（避免 `turnPage` 连点叠多个 timer）；`advanceNyx` 里 `nyxPosition` 上限 `userPosition`（不超车）。
...
翻页 → turnPage(dir)
  → userPosition = clamp(userPosition + dir, 1, total_paragraphs)
  → putProgress(...)（异步写回，fire-and-forget）
  → 前翻（dir=+1）→ evaluateImpulse(bookId, userPosition, userPosition-1)（POST /api/impulse/evaluate，触发冲动，气泡走 SSE 07）
  → 窗口滚动到 userPosition 越界时重新 getBookParagraphs 拉下一窗（§5 窗口规则）
  → startCatchup()（拉开差距 Nyx 继续追）
```

- **窗口规则**：`WINDOW_SIZE = 50`（每窗段数，decision 可推翻）。`openBook` 拉 `[user_position, user_position+WINDOW_SIZE-1]`；`turnPage` 到窗口边界（`userPosition` 超出 `[windowFrom, windowFrom+WINDOW_SIZE-1]` 的 80%）时重拉以 `userPosition` 为中心的新窗。**请求前 clamp 到 `[1, total_paragraphs]`**（`from=max(1, …)`、`to=min(total_paragraphs, …)`）——后端 20 对 `from<1`/`to>total` 返回 422（越界不截断），前端必须先 clamp，否则书首/书尾窗口会 422。`nyxPosition` 追赶只在窗口内段有字长可算（§4 兜底）。
...
- `readerStore`（`tests/stores.test.ts` 增补）：`loadBooks` 落 `books`；`openBook` mock `getProgress`+`getBookParagraphs` → 会话态 + `totalParagraphs` 正确（从 books 列表项取）、`nyx<user` 时 `startCatchup` 被调；`turnPage` 前翻 → `putProgress` + `evaluateImpulse(bookId, new, new-1)` 被调、回翻 → 不评估；`nyxStatus` 派生三态正确；书首/书尾（`from`/`to` 越界）→ `getBookParagraphs` 请求 clamp 到 `[1, total_paragraphs]` 不越界；`reread` → `putProgress({user_position:1, nyx_position:1, reading_speed})` 复位、`userPosition=nyxPosition=1`。
