...
- 当前已迁移的事务/幂等链：`ActivityLifecycle.start/complete/interrupt` 的活动状态、欲望状态和活动事件同事务提交；`DESIRE_EVAL -> DESIRE_GENERATED` 同事务提交；`desire.observation_state`、`desire.activity_end`、`inner_life.observation_state`、`inner_life.desire_satisfied`、`inner_life.activity_end`、`memory.activity_end`、`inner_life.reflection` 使用 `(event_id, consumer_id)` effect marker 防重放；`MemoryFacade.record_recall` 的升级和 `MEMORY_PROMOTED` 事件同事务提交。`inner_life.reflection` 的 `REFLECTION_DONE` 也在同一事务内追加，提交后才唤醒投递。
