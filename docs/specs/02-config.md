...
def _build(dc: type[_T], raw: dict[str, Any]) -> _T:
    ...
    for f in fields(dc):
        ...
        value = raw[f.name]
        # f.type 是字段注解（无 from __future__ import annotations，故为实际类，Python 3.11+）
        if is_dataclass(f.type):
            if not isinstance(value, dict):
                raise ConfigError(f"{dc.__name__}.{f.name} 必须是映射，得到 {type(value).__name__}")
