...
# 值取 {"expression", "inner_life", "desire", "activity"}。CLOCK_TICK 不在此表（走 TICK_ROUTING）。
ROUTING: dict[EventType, list[str]] = {
    ...
    EventType.DESIRE_SATISFIED:    ["inner_life"],             # 欲望→内在生命唯一耦合点（走事件防成环）
...
# TICK_ROUTING：clock_tick 按 content.tick_type 二次路由（1 个 tick_type → 1 个消费者，非广播）。
            ...
            try:
                ...
                for handler in self._handlers.get(event.type, []):
                    try:
                        ...
                        self._logger.exception("handler 处理事件失败 event_id=%s type=%s", event.id, event.type.value)
    ...
    async def _persist(self, event: Event) -> None:
        async with self._db.lock:
            await self._db.conn.execute(
                "INSERT INTO event_log (id, timestamp, source, type, content, correlation_id) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
                    event.id,
                    event.timestamp,
