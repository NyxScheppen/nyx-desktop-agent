...
- **不作为共同浏览 fallback**：本工具允许普通公网 HTTP(S)，自动请求细节和返回值都不暴露
  完整重定向链，因此不能满足共同浏览“每一跳均为公网 HTTPS”的安全契约。
  `13-browsing-system` 在浏览 Facade 内定义专用 `fetch_public_https()`；它不注册为 Tool，
  本 spec 的 `fetch_url()` 签名、返回值和探索语义不变。
...
- [ ] 单元测试 `tests/test_tools/`：
  ...
  - [ ] **web_fetch**（`test_web_fetch.py`，monkeypatch `fetch_url`/`file_io` + fake bus）：
    ...
    - [ ] 抓取空正文 → 返回 `{"error": ...}`
    - [ ] 不触真实网络
- [ ] 集成测试：无（工具是基础设施，无 Facade 管道；真实绑定归 组合根）
- [ ] E2E 测试：无
