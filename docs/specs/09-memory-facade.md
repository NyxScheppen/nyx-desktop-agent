...
- **当前 Task 2 兼容 edge write**：`MemoryStore.upsert_edge` 已要求 `(from_id, to_id, kind, weight, created_at)`，所以当前 legacy `_build_edges` 仅把旧余弦建边结果写成 `MemoryEdgeKind.SEMANTIC`、`created_at=0.0` 的 typed edge，以维持类型检查和现有行为。这里不是 Task 6 的五类建边、LLM 关系边或度数控制实现；Task 6 会替换本段语义
- **建边机制（Task 6 前临时状态）**：新记忆与已有记忆按 `embedding` 余弦相似度建边，`_EDGE_TOP_K=3`、`weight=相似度`、`s > 0` 才建；`embed=None` 或新记忆无 embedding → 跳过。store 会 canonicalize 端点，`MemoryGraph` 无向所以原始调用方向无关
