...
- **前置依赖**：01-types（`DesireType` / `DesireStatus` / `DesireValue` / `ShortTermDesire` / `LongTermDesire` / `Goal` / `DesireState` / `GoalAction` / `Event` / `EventType` / `Source`）、02-config（`DesireConfig`：`peak_threshold` / `retry_limit` / `value_decay`）、03-llm（`LlmClient.complete`）、05-module-bus-system（`Database`、`EventBus`、`short_term_desire` / `desire_value` / `long_term_desire` 三表）、10-desire-value（`decay_value` / `apply_pressure` / `reinforce_weight` / `raise_suppression` / `at_peak` / `is_expressible` / `default_value` / `REFUND_DELTA`）、eval（`Evaluator`）
- **本 spec 带来的连锁改动（ripple，已同步）**：01-types 给 `LongTermDesire` 加 `type` 字段、`DesireValue` 加 `updated_at` 字段；05-module-bus-system 给 `long_term_desire` 加 `type` 列、`desire_value` 加 `updated_at` 列；10 的 `default_value` 补 `updated_at=0.0`；tech-ref §7 补 `desire/store.py`。
...
- **新文件**：`nyx/desire/store.py`、`nyx/desire/lifecycle.py`、`nyx/desire/facade.py`（无 API、无数据变更——表结构是 05-module-bus-system 的活）
