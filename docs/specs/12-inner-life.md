...
- **前置依赖**：01-types（`CurrentState` / `SelfNarrative` / `Personality` / `Values` / `Aesthetic` / `Event` / `EventType` / `Source` / `EnergyState` / `EmotionCategory` / `ActivityType` / `LongTermDesire` / `ReflectionOutcome`）、02-config（`Config` / `DesireConfig.long_term_capacity`）、03-llm（`LlmClient.complete`）、05-module-bus-system（`Database`、`EventBus`、`personality` / `value_system` / `aesthetic` / `energy` / `self_narrative` 五表）、07-memory-system（`MemoryFacade.list_memories`）、11-desire（`DesireFacade.get_pending` / `get_all` / `add_long_term` / `pressure_creation`）、14-activity（`ActivityFacade.get_current`）、eval（`Evaluator`）**
- **联动契约**：11-desire 提供 `add_long_term` / `pressure_creation`；05-module-bus-system 提供五张内在生命单行表及事件事务；07-memory-system 提供 `count_new("reading", since)`；17-expression 消费 `CurrentState.aesthetic`；22-reading-notes 产生 `tag='reading'` 记忆。
...
- **五张单行表**：`personality`、`value_system`、`aesthetic`、`energy`、`self_narrative` 都用 `id='self'`（05-module-bus-system 固定键）。`get_*` 返回 `X | None`（未 seed 时 None）；所有读路径遇 None 抛 `RuntimeError`（"未初始化，组合根必须先 seed"），不静默兜底默认值
- **情感不持久化（design §4.5）**：valence/arousal 在 `InnerLifeFacade` 内存字段（`self._valence` / `self._arousal` / `self._emotion_updated_at`），重启从基线（0,0）重启。没有 emotion 表（05-module-bus-system 无此表）
...
- [ ] **联动已同步**：11-desire 的长期欲望入口、05-module-bus-system 的五张单行表、07-memory-system 的 `count_new`、17-expression 的审美 prompt、22-reading-notes 的 `tag='reading'` 记忆均按当前源码接线
