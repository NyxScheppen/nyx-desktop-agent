...
- **六种活动执行分派（`_run_activity`）**：
  ...
  - `FREE_EXPLORATION`：调 `Exploration.run()`（线性：搜 → 抓正文 → 一次总结，topic = 欲望描述）→ result `{type, outcome, summary, core_discovery, knowledge, new_topics, strong_new_topics, findings}`
