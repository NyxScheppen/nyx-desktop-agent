...
- [ ] `facade.py` 含 `ExpressionFacade`（`reply` / `initiate_chat` / `mutter`）；`pipeline.py` 含 `ReplyState` + `build_reply_graph`；`mutter.py` 含 `MutterCategory` + `_MUTTER_TEMPLATES`（**四类各 10 条**）+ `pick_mutter_category` / `pick_mutter_template` + `should_initiate_chat`（实现见 `nyx/expression/mutter.py` 等源文件）
