...
- [ ] **15 个端点**：tech-ref §4 的 14 个 REST + `GET /api/events`（SSE）；除 upload/materials/books 外每个 REST 端点 = 对应 Facade 读方法的薄封装（无额外业务逻辑）
...
- **`_App` 内部 dataclass**：组合根的装配产物（不是新增抽象层——不增 Facade→子系统→内部类之外的层），持有 8 个组件引用 + 4 个运行期状态（`last_chat_at`/`last_presence`/`last_window_title`/`last_screen_summary`）+ 视觉装配 `screen_observer`（`vision.enabled` 时），端点/handler 闭包捕获 `_App` 实例
