...
- [ ] `nyx/reading/store.py` 含 `ReadingStore(db: Database)`：`insert_book_with_paragraphs(title, author, filename, content_hash, segments: list[Segment]) -> tuple[Book, bool]`（原子：单事务插 book + paragraphs；`content_hash` 撞唯一索引回滚返已存在书、`bool=False`）/ `find_by_hash(content_hash) -> Book | None`（`async`、用注入 `Database` 的 `conn`+`lock`）
...
- `ReadingStore`（`nyx/reading/store.py`）——books/paragraphs 的 DB 读写，唯一写路径；方法：`insert_book_with_paragraphs(title, author, filename, content_hash, segments: list[Segment]) -> tuple[Book, bool]`（`Segment.is_chapter_start` 落 `paragraphs.is_chapter_start`，bool 存 1/0；读回时 `bool(...)` 还原，读侧在 20；单事务 `BEGIN`/`COMMIT`/`ROLLBACK` 原子写 book+paragraphs，`executemany` 批量插段；`content_hash` 撞唯一索引 → 回滚 → 返已存在书、`bool=False`）、`find_by_hash(content_hash) -> Book | None`
- `ReadingFacade`（`nyx/reading/facade.py`）——`import_book(filename, data)`：`parse_epub` → 空正文 `ValueError` → `insert_book_with_paragraphs`（去重/落库在 store 内原子完成）→ `bool=False` 时抛 `DuplicateBookError`（域异常，端点映射 409）
...
- **去重靠 `content_hash`**：`content_hash = sha256("\n".join(所有段落文本))`（对分段后正文哈希，改名重导仍命中）；`books.content_hash` 加**唯一索引** `idx_books_content_hash`（v8 迁移把 v7 的普通索引升级为 UNIQUE——并发导入同内容时不产重复书；store 靠唯一索引的 `IntegrityError` 判重回滚、返已存在书，Facade 据此映射 409）。
...
- `POST /api/books`：multipart `UploadFile`，字段名 `file`（`file: UploadFile = File(...)`，对齐 `/api/upload` 同名字段）。端点内顺序：
  1. `_sanitize_filename(file.filename or "book.epub")` 消毒：剥路径（`Path(name).name`）、去控制字符与 `< > "`、限长 255、空回退 `book.epub`；
  2. 扩展名非 `.epub`（`Path(filename).suffix.lower() != ".epub"`）→ 400；
  3. 分块读入 `bytearray`，累计超 `_MAX_EPUB_BYTES` → 400「文件过大」（不整读超限文件）；
  4. `import_book(filename, data)`：`DuplicateBookError` → 409 `{"existing_book_id", "title"}`；`ValueError`（空正文 / 无效 EPUB / 解压超限）→ 400（`detail=str(e)`）；其它异常（含 DRM）→ `logging.getLogger(__name__).exception(...)` 记日志 + 500「EPUB 解析失败」；
  5. 成功 201 返回 `Book` 的 JSON（dataclass 直序列化，无 StrEnum 字段）。
