...
- **冷却用单调钟 `time.monotonic()`、不用墙钟 `time.time()`**：冷却时间戳是纯内存态、不落库、不跨重启，墙钟跳变（NTP 校时前跳 / 用户改系统时间 / 休眠唤醒 RTC 校正）会让 `now - last_at` 失真——前跳使冷却提前失效、后跳使冷却异常拉长。单调钟只保证单调递增，正合冷却语义。落库的 `created_at`/`updated_at`（`nyx/reading/store.py`）仍用 `time.time()`（需跨重启），别一刀切。
- **分派为 best-effort、不阻塞端点**：mutter/提问的 LLM 与 `associate` 的记忆检索在后台任务里跑，`evaluate_paragraph` 同步返回 `triggered`（复合行为列表）。LLM 空/失败只记日志、不广播、不反噬翻页主流程。每处 `llm.complete` 后调 `evaluator.evaluate(output)`（mutter + 4 提问有 LLM 故 evaluate；`associate` 无 LLM 不 evaluate）。后处理（`output.content.strip()` / `memories[:3]`）与 `bus.publish` 一并收进 try；后台任务 `asyncio.create_task` 挂 `add_done_callback` 兜底记逃逸异常，确保契约违约（content/search 返回 `None`）也只记日志、不崩不反噬。
