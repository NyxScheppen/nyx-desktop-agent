...
- **前置依赖**：01-types（TypedDict 约定）、05-module-bus-system（`_MIGRATIONS` v11 + v12 `first_created_at`）、07-memory-system（`count_new(tag | None, since)` 计数）、12-inner-life（`Reflection.run`/`get_state`）、22-reading-notes（`remember_reading` 落 `tag='reading'`）
