...
- 后端测试文件：65
- `pytest --collect-only -q`：1007 tests collected
- 最近一次全量验证：`1006 passed, 1 skipped`
- 后端运行：`pytest -q`
...
| 记忆 | `tests/test_memory/` | 7 | kind/topics、精确与语义去重、episode 保守去重、ANN、融合召回、联想图、Facade、实体事实有效期/替代/幂等/冲突折叠/来源过滤/有界召回、durable 活动记忆不持锁等待 embedding |
