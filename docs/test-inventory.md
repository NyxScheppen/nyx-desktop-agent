...
| `test_exploration_run_no_web` | 功能正确 | `run` 返回 `{findings, notes, nodes}`、步数 == `_MAX_STEPS`、`correlation_id` 传递、每次 complete 后 evaluate、图不含 web_search |
...
| `test_search_web_downloads_first_result` | 功能正确 | `_search_web` 搜到结果 → 调 `web_fetch` 下第一条正文，`findings` 追加「已下载资料：…」 |
| `test_search_web_no_crash_when_download_fails` | 边界鲁棒 | `web_fetch` 抛异常 → 静默吞掉（best-effort），探索不崩、findings 仍有搜索结果 |
